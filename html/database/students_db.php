...
function addStudent(Student $student)
{
  ...
  // And minors
  $smt = $pdo->prepare("INSERT INTO $student_minor_tbl (student_id, minor_id) SELECT $student_tbl.id, $minor_tbl.id FROM $student_tbl INNER JOIN $minor_tbl WHERE $student_tbl.email=:email AND $minor_tbl.minor IN ($minors)");
  $smt->bindParam(":email", $student->email, PDO::PARAM_STR);
  $smt->execute();
}
...
function printTbl()
{
  ...
  print_r($pdo->query("SELECT * FROM $student_tbl")->fetchAll(PDO::FETCH_ASSOC));
}
